...
class SimpleHandTouchDetector:
    ...
    def detect_hand_touches(self, frame):
        ...
        if self.cashier_stable_id is not None:
            ...
            # If locked cashier not found in this frame, skip frame but KEEP lock
            if cashier is None:
                return frame, []
        else:
            ...
            if len(people) == 0:
                return frame, []  # No people to lock
            ...
            if cashier is not None:
                ...
                return frame, []  # Should never happen but safety check
        
        # If no cashier identified (no people detected), keep previous position
        if cashier is None:
            return frame, []
        
        # If no cashier identified (no people detected), skip
        if cashier is None:
            return frame, []
...
def main():
    ...
    if not videos:
        print("❌ No videos found in input/videos/")
        return
